...
log_and_echo "│           на HQ-SRV (192.168.1.10) для портов 8080 и 2026    │"
log_and_echo "└──────────────────────────────────────────────────────────────┘"
...
echo "└──────────────────────────────────────────────────────────────┘"
echo ""
echo "  Критерий 16 (DNAT 8080):    $(grep -qF 'PREROUTING -i enp7s1 -p tcp -m tcp --dport 8080 -j DNAT --to-destination 192.168.1.10:80' $IPTABLES_CONF 2>/dev/null && echo '✓ OK' || echo '✗ FAIL')"
echo "  Критерий 16 (DNAT 2026):    $(grep -qF 'PREROUTING -i enp7s1 -p tcp -m tcp --dport 2026 -j DNAT --to-destination 192.168.1.10:2026' $IPTABLES_CONF 2>/dev/null && echo '✓ OK' || echo '✗ FAIL')"
echo "  Критерий 16 (FORWARD):      $(grep -qF 'FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT' $IPTABLES_CONF 2>/dev/null && echo '✓ OK' || echo '✗ FAIL')"
echo ""
